...
1. 打开 Hammerspoon，按提示授予「辅助功能」权限
   （系统设置 → 隐私与安全性 → 辅助功能）。
2. 编辑 `~/.hammerspoon/init.lua`，把顶部的 `PEER` 改成对方 Mac 的 `.local`
   主机名。可在对方 Mac 上运行 `scutil --get LocalHostName`，假设输出
...
# 编辑 send.sh 顶部的 PEER 为 Mac 的局域网 IP 或 .local 主机名
```
...
- 推荐在 Mac 之间使用 `.local` 主机名，避免 DHCP 导致 IP 变化。
- 部分 Android 环境无法稳定解析 `.local`，此时请使用局域网 IP，并在路由器中配置 DHCP 静态租约。
